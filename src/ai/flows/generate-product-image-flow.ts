...
const generateProductImageFlow = ai.defineFlow(
  ...
  async (input): Promise<GenerateProductImageOutput> => {
    try {
    ...
    } catch (err: unknown) { // Changed to unknown for safer type handling
      // Log the full error on the server for debugging
      console.error('Error in generateProductImageFlow. Raw error object:', JSON.stringify(err, Object.getOwnPropertyNames(err instanceof Error ? err : {})));
      ...
      if (err instanceof Error && err.message) {
        ...
        rawErrorMessage = 'An unknown error structure was caught.';
      }
      ...
      if (lowerCaseErrorMessage.includes('image generation is not available in your country')) {
          clientErrorMessage = 'AI image generation failed: The selected AI model (gemini-2.0-flash-exp) for image generation is not available in your current country/region. Please check Google Cloud documentation for model availability. If deploying to Vercel, consider adjusting your Vercel Function deployment region if possible.';
      } else if (lowerCaseErrorMessage.includes('api key not valid') || lowerCaseErrorMessage.includes('api_key_invalid')) {
        clientErrorMessage = 'AI image generation failed: The API key is not valid. Please check server configuration (e.g., GOOGLE_API_KEY environment variable in Vercel/production) and ensure it is correctly set and has permissions for the Generative Language API or Vertex AI API.';
      ...
      } else if (lowerCaseErrorMessage.includes('api_not_enabled') || lowerCaseErrorMessage.includes('service not enabled') || lowerCaseErrorMessage.includes('generative language api is not used') || lowerCaseErrorMessage.includes('vertex ai api is not used')) {
          ...
          clientErrorMessage = 'AI image generation failed due to a permissions issue. Please ensure the service account or API key used by your Vercel Functions/Cloud Run has the necessary IAM roles (e.g., Vertex AI User, AI Platform User, or Generative Language API User).';
      ...
      } else if (rawErrorMessage && rawErrorMessage !== 'An unknown error structure was caught.') {
        ...
        const errorString = JSON.stringify(err, Object.getOwnPropertyNames(err instanceof Error ? err : {}));
        clientErrorMessage = 'An unexpected and unknown error occurred during AI image generation. Administrators: Please check server logs for any available details. The raw error object was: ' + (errorString.length > 200 ? errorString.substring(0, 200) + '...' : errorString) ;
      }
